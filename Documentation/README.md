...
Group      specificity 10    TavernContext        shared by NPCs in the tavern
Type       specificity 20    GuardKnowledge       shared by all guards
Individual specificity 100   this NPC's NPCContext private to one NPC
```
...
providers at *equal* specificity are peers — a same-key collision between them is a
...
your own sources — quest logs, relationships, inventory, factions — and drop them into
...
Swap providers by assigning a different one to `StreamHandler`'s `Provider Component` field — no other code changes.
...
A new provider that needs a new setting (an org ID, an auth header, a region) adds that field to *its own* component — `LLMConfig` and the rest of the SDK stay untouched.
